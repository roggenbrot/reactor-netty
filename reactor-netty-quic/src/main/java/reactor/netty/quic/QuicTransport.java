...
abstract class QuicTransport<T extends Transport<T, CONF>, CONF extends QuicTransportConfig<CONF>>
	 ...
	 * @return a new {@link QuicTransport} reference
	 */
	...
	public final T initialSettings(Consumer<QuicInitialSettingsSpec.Builder> initialSettings) {
		...
		QuicInitialSettingsSpec.Builder builder = QuicInitialSettingsSpec.builder();
	...
	public final <A> T streamAttr(AttributeKey<A> key, @Nullable A value) {
		...
		T dup = duplicate();
		dup.configuration().streamAttrs = QuicServerConfig.updateMapInternal(configuration().streamAttrs, key, value);
		return dup;
	}
	...
	public final <A> T streamOption(ChannelOption<A> key, @Nullable A value) {
		...
		T dup = duplicate();
		dup.configuration().streamOptions = QuicServerConfig.updateMapInternal(configuration().streamOptions, key, value);
		return dup;
	}
